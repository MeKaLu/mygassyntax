...
hook global BufCreate .*\.(s|S|asm)$ %{
    set-option buffer filetype gassy
}

hook global WinSetOption filetype=gassy %{
    require-module gassy

    hook window ModeChange pop:insert:.* -group gassy-trim-indent gassy-trim-indent
    hook window InsertChar \n -group gassy-indent gassy-indent-on-new-line
    hook -once -always window WinSetOption filetype=.* %{ remove-hooks window gassy-.+ }
}

hook -group gassy-highlight global WinSetOption filetype=gassy %{
    add-highlighter window/gassy ref gassy
    hook -once -always window WinSetOption filetype=.* %{ remove-highlighter window/gassy }
}


provide-module gassy %{

add-highlighter shared/gassy regions
add-highlighter shared/gassy/code default-region group
add-highlighter shared/gassy/string         region '"' (?<!\\)(\\\\)*"        fill string
add-highlighter shared/gassy/commentMulti   region /\*       \*/              fill comment
add-highlighter shared/gassy/commentSingle1 region '@'       '$'              fill comment
add-highlighter shared/gassy/commentSingle2 region '//'       '$'              fill comment

# Constant
add-highlighter shared/gassy/code/ regex (0[xX][0-9a-fA-F]+|\b[0-9]+)\b 0:value

# Labels
add-highlighter shared/gassy/code/ regex ^\h*([A-Za-z0-9_.-]+): 0:operator

# ARM Directives
add-highlighter shared/gassy/code/ regex ((^|\s+)\.([248]byte|align|arch(_extension)?|arm|bsscantunwind|code|[cf]pu|[dq]n|eabi_attribute|even|extend|ldouble|fnend|fnstart|force_thumb|handlerdata|inst(\.[nw])?|ltorg|movsp|object_arch|packed|pad|personality(index)?|pool|req|save|setfp|screl32|syntax|thumb(_func|_set)?|tlsdescseq|unreq|unwind_raw|vsave)(\h+|$)) 0:type

# Assembler Directives
add-highlighter shared/gassy/code/ regex ((^|\s+)\.(abort|ABORT|align|app-file|ascii|asciz|balign[wl]|byte|comm|data|def|desc|dim|double|eject|else|endif|equ|extern|file|fill|float|global|globl|hword|ident|if|include|int|irp|irpc|lcomm|iflags|line|linkonce|ln|mri|list|loc|local|long|macro|nolist|octa|org|print|purgem|p2align[wl]?|psize|quad|rept|sbttl|section|set|short|single|size|skip|space|stab[dns]|string|struct|tag|text|title|type|title|uleb128|val|vtable_entry|weak|word|rodata|zero)(\h+|$)) 0:type

# Registers
add-highlighter shared/gassy/code/ regex \%(([re](ax|bx|cx|dx|si|di|bp|sp))|(al|bl|cl|dl|sil|dil|bpl|spl)|(r[8-9][dwb])|(r1[0-5][dwb])|(cs|ds|es|fs|gs|ss|ip|eflags)|([xy]mm[0-9]|[xy]mm1[0-5]))\b 0:variable

# General Instructions
add-highlighter shared/gassy/code/ regex \
...
define-command -hidden gassy-trim-indent %{
...
define-command -hidden gassy-indent-on-new-line %~
    evaluate-commands -draft -itersel %<
        # preserve previous line indent
        try %{ execute-keys -draft <semicolon> K <a-&> }
        # filter previous line
        try %{ execute-keys -draft k : gassy-trim-indent <ret> }
        # indent after label
